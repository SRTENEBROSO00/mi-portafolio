        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className='grid md:grid-cols-2 lg:grid-cols-3 gap-8  '>
            ...
            {projectData.map((projectGroup, index) => (
              <div key={index} className='group h-96 flex flex-col justify-around items-start bg-black/40 backdrop-blur-sm border border-white/10 rounded-xl overflow-hidden hover:border-orange-500/50 transition-all hover:shadow-orange-500/10 hover:shadow-lg'>
                <div>
                ...
                <div className=' pt-6'>
                  ...
                  <p className='text-muted-foreground mb-'> {projectGroup.decription}</p>
                </div>
                <div className=''>
                  {projectGroup.badge.map((filtrar, filterIndex) => (
                    <Badge key={filterIndex} className='className="bg-orange-950/30 text-orange-500 border-orange-800 hover:bg-orange-950/50 m-2 text-sm p-1 border inline-block rounded-full px-3 '>
                      {filtrar}
                    </Badge>
                  ))}
                </div>
